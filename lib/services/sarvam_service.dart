...
class SarvamService {
  /// Calls Sarvam AI and returns {'prediction': ..., 'strategy': ...}.
  static Future<Map<String, String>> generatePredictionAndStrategy({
    ...
    if (apiKey == null || apiKey.isEmpty) {
      developer.log('[SarvamService] Error: API Key missing from .env');
      return _fallback('API Key Missing');
    }

    // 2. THE CORRECT SARVAM ENDPOINT
    final endpoint = 'https://api.sarvam.ai/v1/chat/completions';

    developer.log('[SarvamService] Calling Sarvam for route: $source -> $destination');
    ...
    try {
      final response = await http.post(
        ...
        headers: {
          ...
          'api-subscription-key': apiKey, // Using the required header format for Sarvam API (some use Authorization: Bearer, wait)
        },
        body: body,
      );

      developer.log('[SarvamService] Status: ${response.statusCode}');
      ...
      developer.log('[SarvamService] Exception: $e');
      return _fallback('Network/Parsing Error');
    }
  }

  /// Chat Logic using Sarvam REST API
  static Future<String> sendChatMessage(String userMessage) async {
    try {
      ...
      // 2. The Sarvam completion endpoint
